#!/usr/bin/env bash
# Single entry point for the PyPI release verification stack.
#
# Same observability harness as deploy/source/ but builds the app from the
# published wheel on PyPI instead of the workspace source tree. Pin the
# version with TFA_VERSION (default in docker-compose.yml).
#
...
Overridable env vars:
  TFA_VERSION       PyPI version of transparent-fastapi to install (default
                    in docker-compose.yml — bump per release)
...
cmd_up() {
  require_cmd docker
  resolve_compose
  log "starting docker compose stack (transparent-fastapi==${TFA_VERSION:-default})..."
